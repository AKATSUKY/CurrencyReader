...
def addNewCurrencyValue(url): 
    ...
    objs = response.json()
    quotes = objs["quotes"]
    currencies = session.query(Currency).filter_by(stopGetValue=None)
    .all()
    for currency in currencies: 
        newCurrencyValue = CurrencyValue(timestamp = 
        datetime.utcfromtimestamp(objs['timestamp']), 
        value = quotes['USD' + currency.code], 
        currencyId = currency.id)   
        if session.query(CurrencyValue).filter_by
        (timestamp=newCurrencyValue.timestamp, 
        currencyId=newCurrencyValue.currencyId).first() == None:
...
if args.c == 'c':
    ...
    for currency in objs["currencies"]:
        ...
        if session.query(Currency).filter_by(code=auxCurrency.code)
        .first() == None:
    ...
    for i in range(delta.days):
        ...
        url = 'http://apilayer.net/api/historical?access_key=
        &source=USD&format=1&date='
        + date
        addNewCurrencyValue(url)
else:
    url = 'http://apilayer.net/api/live?access_key=
    &source=USD&format=1'
    addNewCurrencyValue(url)
